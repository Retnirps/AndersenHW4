package com.majestadev.andersenhw4
...
class AnalogClockView @JvmOverloads constructor(
    ...
    init {
        ...
        try {
            circleColor = typedArray.getColor(R.styleable.AnalogClockView_circleColor, 0)
            secondHandColor = typedArray.getColor(R.styleable.AnalogClockView_secondHandColor, 0)
            minuteHandColor = typedArray.getColor(R.styleable.AnalogClockView_minuteHandColor, 0)
            hourHandColor = typedArray.getColor(R.styleable.AnalogClockView_hourHandColor, 0)
            circleStrokeWidth = typedArray.getFloat(R.styleable.AnalogClockView_circleStrokeWidth, 0f)
            numeralSpacing = typedArray.getInt(R.styleable.AnalogClockView_numeralSpacing, 0)
            numeralsFontSize = typedArray.getInt(R.styleable.AnalogClockView_numeralsFontSize, 0)
            secondHandWidth = typedArray.getFloat(R.styleable.AnalogClockView_secondHandWidth, 0f)
            otherHandsWidth = typedArray.getFloat(R.styleable.AnalogClockView_otherHandsWidth, 0f)
            numeralsFontWidth = typedArray.getFloat(R.styleable.AnalogClockView_numeralsFontWidth, 0f)
        } finally {
            typedArray.recycle()
        }
    }
    ...
    private fun drawHand(canvas: Canvas?, loc: Float, isHour: Boolean, isSecond: Boolean) {
        ...
        when {
            isHour -> {
                canvas?.drawLine(width / 2f, height / 2f, (width / 2 + cos(angle) * handRadius).toFloat(), (height / 2 + sin(angle) * handRadius).toFloat(), paintHourHand)
            }
            isSecond -> {
                canvas?.drawLine(width / 2f, height / 2f, (width / 2 + cos(angle) * handRadius).toFloat(), (height / 2 + sin(angle) * handRadius).toFloat(), paintSecondHand)
            }
            else -> {
                canvas?.drawLine(width / 2f, height / 2f, (width / 2 + cos(angle) * handRadius).toFloat(), (height / 2 + sin(angle) * handRadius).toFloat(), paintMinuteHand)
            }
        }
    }
